...
global $mainframe;
$mainframe->addCustomHeadTag( '<link rel="stylesheet" href="administrator/templates/system/css/system.css" type="text/css" />' );
$mainframe->addCustomHeadTag( '<link href="administrator/templates/khepri/css/template.css" rel="stylesheet" type="text/css" />' ); 
$mainframe->addCustomHeadTag( '<link rel="stylesheet" type="text/css"  href="components/com_joomsport/css/admin_bl.css" />' );
$mainframe->addCustomHeadTag( '<link href="administrator/templates/khepri/css/icon.css" rel="stylesheet" type="text/css" />' ); 
?>
<?php
...
class BLeagueControllerAdmin extends JController
{
	...
	function team_save($apl = 0){
		...
		if(isset($_POST['photos_id']) && count($_POST['photos_id'])){
			...
			for($i = 0; $i < count($_POST['photos_id']); $i++){
				...
				$photo_name = stripslashes(strval($_POST['ph_names'][$i]));
				...
				$query = "UPDATE #__bl_photos SET ph_name = '".mysql_real_escape_string($photo_name)."' WHERE id = ".$photo_id;
		...
		if($apl){
			$link = "index.php?option=com_joomsport&controller=admin&task=team_edit&cid[]=".$row->id."&sid=".$s_id;
		}else{
			$link = "index.php?option=com_joomsport&controller=admin&task=admin_team&sid=".$s_id;
		}
	...
	function team_del(){
		...
		//$mainframe->redirect( JRoute::_("index.php?option=com_joomsport&controller=admin&task=admin_team&sid=".$s_id));
	...
	function player_save($apl = 0){
		...
		if($apl){
			$link = "index.php?option=com_joomsport&controller=admin&task=player_edit&cid[]=".$row->id."&sid=".$s_id.'&msg='.$msg;
		}else{
			$link = "index.php?option=com_joomsport&controller=admin&task=admin_player&sid=".$s_id;
		}
	...
	function player_del(){
		...
		$this->setRedirect( JRoute::_("index.php?option=com_joomsport&controller=admin&task=admin_player&sid=".$s_id));
	}
	...
	if(isset($_POST['home_team']) && count($_POST['home_team'])){
		foreach($_POST['home_team'] as $home_team){
			...
			$match->m_time = strval($_POST['match_time'][$mj])." ".strval($_POST['match_time_am'][$mj]);
		...
		if(!$isapply){
			...
			$link = JRoute::_("index.php?option=com_joomsport&controller=admin&task=admin_matchday&sid=".$s_id);
		}else{
			$link = JRoute::_("index.php?option=com_joomsport&controller=admin&task=matchday_edit&sid=".$s_id."&cid[]=".$row->id);
		}
	...
	function matchday_del(){
		...
		$s_id = JRequest::getVar( 'sid', 0, '', 'int' );	
		$this->setRedirect( JRoute::_("index.php?option=com_joomsport&controller=admin&task=admin_matchday&sid=".$s_id));
	}
	...
	function admin_match()
	{
		...
		$this->setRedirect( JRoute::_("index.php?option=com_joomsport&controller=admin&task=matchday_edit&sid=".$s_id."&mid=".$mid));
	}
	...
	if(isset($_POST['t2_squard_res']) && count($_POST['t2_squard_res'])){
		...
		$isapply = JRequest::getVar( 'isapply', 0, '', 'int' );
		if(!$isapply){
			$this->setRedirect( JRoute::_("index.php?option=com_joomsport&controller=admin&task=matchday_edit&cid[]=".$row->m_id."&sid=".$s_id));
		}else{
			$this->setRedirect( JRoute::_("index.php?option=com_joomsport&controller=admin&task=match_edit&cid[]=".$row->id."&sid=".$s_id));
		}
	} 
	...
	*/
		...
		$this->setRedirect( JRoute::_("index.php?option=com_joomsport&controller=admin&task=matchday_edit&cid[]=".$row->id."&sp_id=".$sp_id."&msg=".$msg));
	} 
