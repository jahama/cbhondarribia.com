...
class bleagueViewmatch extends JView
{
	function display($tpl = null)
	{
		...
		$query = "SELECT me.*,ev.*,CONCAT(p.first_name,' ',p.last_name) as p_name,p.team_id,p.id as plid FROM #__bl_match_events as me, #__bl_events as ev, #__bl_players as p WHERE me.player_id = p.id AND ev.player_event = '1' AND me.e_id = ev.id AND me.match_id = ".$m_id." AND me.t_id=".$match->aw_id." ORDER BY me.minutes";
		$db->setQuery($query);
		//echo mysql_error();die();
