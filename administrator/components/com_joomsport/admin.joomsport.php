...
function BL_MatchSave($option){
	...
	$row->m_time = JRequest::getVar( 'd_time', '', 'post', 'string', JREQUEST_ALLOWRAW ).' '.$_POST['da_time'];
...
function BL_EventSave($option){
	...
	if(isset($_FILES['new_event_img']['name']) && $_FILES['new_event_img']['tmp_name'] != '' && isset($_FILES['new_event_img']['tmp_name'])){
		$bl_filename = strtolower($_FILES['new_event_img']['name']);
		
		$bl_filename = str_replace(" ","",$bl_filename);
		//echo $bl_filename;
		$baseDir =  JPATH_ROOT . '/media/bearleague/events/' ;
		 uploadFile($_FILES['new_event_img']['tmp_name'], $bl_filename, $baseDir);
...
function BL_saveLanguage( $option ) {
	...
	if (JRequest::getCmd('task') == 'apply_lang') {
		$mainframe->redirect( "index2.php?option=$option&task=edit_lang&hidemainmenu=1&cid[]=". $row->id );
	} else {
		$mainframe->redirect( "index2.php?option=$option&task=languages" );
	}
}
